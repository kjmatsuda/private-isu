...
module Isuconp
  class App < Sinatra::Base
    ...
    MEMCACHED = Dalli::Client.new(ENV['ISUCONP_MEMCACHED_ADDRESS'])
      ...
      def make_posts(results, all_comments: false)
        ...
        results.to_a.each do |post|
          ...
          if cached_comments
            ...
            # 存在しなかったら MySQL にクエリ
            # JOIN で1クエリで取得する
            query = 'SELECT c.`comment`, c.`created_at`, u.`account_name` FROM `comments` c JOIN `users` u ON c.`user_id`=u.`id` WHERE `post_id` = ? ORDER BY `created_at` DESC'
            unless all_comments
              query += ' LIMIT 3'
            end
 ...
 WHERE u.del_flg=0 ORDER BY p.created_at DESC LIMIT #{POSTS_PER_PAGE}")
    ...
    get '/@:account_name' do
      ...
      results = db.xquery("SELECT p.id, p.user_id, p.body, p.created_at, p.mime, u.account_name
        FROM `posts` AS p JOIN `users` AS u ON (p.user_id=u.id)
       WHERE p.user_id = ? AND u.del_flg=0 ORDER BY p.created_at DESC LIMIT #{POSTS_PER_PAGE}",
        user[:id]
      )
    ...
    get '/posts' do
      ...
      results = db.xquery("SELECT p.id, p.user_id, p.body, p.created_at, p.mime, u.account_name
        FROM `posts` AS p JOIN `users` AS u ON (p.user_id=u.id)
       WHERE p.created_at <= ? AND u.del_flg=0 ORDER BY p.created_at DESC LIMIT #{POSTS_PER_PAGE}",
    ...
    get '/posts/:id' do
      results = db.xquery("SELECT p.id, p.user_id, p.body, p.created_at, p.mime, u.account_name
        FROM `posts` AS p JOIN `users` AS u ON (p.user_id=u.id)
       WHERE p.id = ? AND u.del_flg=0 ORDER BY p.created_at DESC LIMIT #{POSTS_PER_PAGE}",
        params[:id]
      )
    ...
    post '/' do
      ...
      if params['file']
        ...
        # アップロードされたテンポラリファイルを mv して配信ディレクトリに移動
